...
- (void)updateIndicators {
    ...
    }else if(mode == MBProgressHUDModeCancel && self.customView != nil)
    {
        self.indicator = self.customView;
    }else {
#if __has_feature(objc_arc)
...
//    id me = [self initWithFrame:CGRectMake(0, 0, appDelegate.screenWidth, appDelegate.screenHeight)];

    id me = nil;
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0 )
        me = [self initWithFrame:CGRectMake(0, 64, view.frame.size.width, appDelegate.screenHeight)];
    else
        me = [self initWithFrame:CGRectMake(0, 0, view.frame.size.width, appDelegate.screenHeight)];
...
- (void)drawRect:(CGRect)rect {
	...
	// Corner radius
	float radius = 4.0f;
    //	radius = (mode == MBProgressHUDModeCancel ? 0 : radius);//add by wj
